...
import {HOME_PAGE_SECTIONS, LIST_CHAPTER_BY_STORY_ID, PREF_KEYS,} from "./constants";
...
export class Controller {
    ...
    async getContent(contentId: string) {
        const domain = await GlobalStore.getDomain()
        const storyId = contentId.split('-').pop()
        const [content, chapters] = await Promise.all(
            [
                this.fetchHTML(contentId),
                this.fetchHTML(`${domain}${LIST_CHAPTER_BY_STORY_ID}?StoryId=${storyId}`)
            ]
        )
        return this.parser.getContent(content, chapters, contentId);
    }
